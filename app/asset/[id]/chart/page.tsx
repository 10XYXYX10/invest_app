...
export async function generateMetadata({ params }: PageProps<"/asset/[id]/chart">): Promise<Metadata> {
  const { id } = await params;
  const asset = ASSET_BY_ID.get(id);
  return { title: asset ? `${asset.name} チャート` : "チャート" };
}

export default async function Page({ params }: PageProps<"/asset/[id]/chart">) {
  const { id } = await params;
  const asset = ASSET_BY_ID.get(id);
  if (!asset) notFound();
  return <ChartClient asset={asset} />;
}
